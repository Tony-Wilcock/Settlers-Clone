...
public class NodeManager2 : MonoBehaviour
{
    ...
    private ConcurrentDictionary<int, bool> nodeValidityCache = new ConcurrentDictionary<int, bool>(); // Update this too if using Suggestion 1 from PathFindingManager
    ...
    public void Initialise(HexGridManager manager, HexGridSettings settings) // Constructor (optional: if needed)
    {
        ...
        InitialsiePools(); // Initialize the object pool at startup
    }

    private void InitialsiePools()
    {
    ...
    private GameObject GetFlagFromPool()
    {
        ...
        else
        {
            ...
            Debug.LogWarning("NodeManager2: Pool is empty, instantiating new flag (consider increasing pool size).");
    ...
    private GameObject GetObstacleFromPool()
    {
        ...
        {
            Debug.LogWarning("NodeManager2: Obstacle pool is empty, instantiating new obstacle.");
    ...
    public void SetNodeResourceType(int vertexIndex, ResourceType type, int amount = 0)
    {
        NodeData data = EnsureNodeData(vertexIndex);
    ...
    public void PlaceFlag()
    {
        ...
        if (data.HasPath)
        {
            manager.PathFindingManager.SplitPathAt(heldVertexIndex);
        }
    ...
    public int NumberOfPathsAttachedToNode(int vertexIndex)
    {
        ...
        {
            NodeData neighborData = GetNodeData(neighbor);
            if (neighborData != null && neighborData.HasPath) count++;
        }
        return count;
    }
    ...
    public void TryRemoveFlag()
    {
        ...
        if (data.HasPath)
        {
            switch (NumberOfPathsAttachedToNode(vertexIndex))
            {
                case 1:
                    RemoveFlag(vertexIndex);
                    manager.PathFindingManager.RemovePath();
                    break;
                case 2:
                    RemoveFlag(vertexIndex);
                    manager.PathFindingManager.JoinPathAt(vertexIndex);
                    break;
                default:
                    Debug.LogWarning($"Warning: This will remove all paths connected to node {vertexIndex}.");
