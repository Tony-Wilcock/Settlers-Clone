...
public class HexGridInteraction : MonoBehaviour
{
    ...
    [SerializeField] private BuildingType selectedBuildingType = BuildingType.HQ;

    public void Initialise(HexGridManager manager, PathFindingManager pathFindingManager)
    {
    ...
    {
        if (manager.UIManager.AreAnyPanelsActive() && !pathFindingManager.isInPathCreationMode) return; // Skip if any panel is active
        ...
        if (pathFindingManager.IsInPathCreationMode)
        {
            pathFindingManager.TryAddPathToEndNode(node);
        }
...
                Debug.Log($"Cell: {cellIndex}.{localVertexIndex}");
                return;
            }
        }
    ...
    private string BuildCellDebugInfo(int node, NodeData nodeData)
    {
        ...
        if (nodeData.HasPath) sb.AppendLine($"Has Path with id: {manager.PathFindingManager.GetPathId(node)}");
        ...
        if (nodeData.ResourceType != ResourceType.None) sb.AppendLine($"Resource Type: {nodeData.ResourceType}");
